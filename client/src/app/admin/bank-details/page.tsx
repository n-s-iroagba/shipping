...
export default function BanksPage() {
  ...
  if (error) {
    return <ErrorAlert message={error || "Failed to load banks"} />;
  }

  return (
    <div className="bg-slate-50 min-h-screen p-4 overflow-x-hidden">
      <div className="mx-auto w-full min-w-0 max-w-sm sm:max-w-md md:max-w-lg lg:max-w-2xl xl:max-w-4xl">
        {/* Header Card */}
        <div className="bg-white rounded-2xl shadow-sm border-2 border-slate-100 p-6 mb-6 relative w-full overflow-hidden">
        ...
        {/* Forms */}
        <div className="space-y-6 mb-8 w-full min-w-0">
          {createBank && (
            <div className="bg-white rounded-2xl shadow-sm border-2 border-slate-100 overflow-hidden w-full min-w-0">
              <BankForm
                adminId={adminId}
                onClose={() => setCreateBank(false)}
              />
            </div>
          )}
          ...
          {bankToUpdate && (
            <div className="bg-white rounded-2xl shadow-sm border-2 border-slate-100 overflow-hidden w-full min-w-0">
              <BankForm
                adminId={adminId}
                existingBank={bankToUpdate}
                onClose={() => setBankToUpdate(null)}
              />
            </div>
          )}
        </div>
        ...
        {bank ? (
          <div className="bg-white rounded-2xl shadow-sm border-2 border-slate-100 hover:border-slate-200 transition-colors w-full min-w-0 overflow-hidden">
            <BankCard
              bank={bank}
          ...
          <div className="bg-white rounded-2xl shadow-sm border-2 border-slate-100 p-8 text-center w-full min-w-0">
